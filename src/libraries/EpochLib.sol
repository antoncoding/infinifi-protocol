...
library EpochLib {
    ...
    function epoch(uint256 _timestamp) internal pure returns (uint256) {
        return (_timestamp - EPOCH_OFFSET) / EPOCH;
    }

    /// @notice epoch end (end of the epoch) for a given timestamp
    function nextEpoch(uint256 _timestamp) internal pure returns (uint256) {
        return epoch(_timestamp) + 1;
    }

    /// @notice Convert epoch to timestamp taking into account the epoch offset
    function epochToTimestamp(uint256 _epoch) internal pure returns (uint256) {
